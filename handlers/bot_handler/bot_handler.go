...
func MainHandler(c *gin.Context) {
	events, err := utils.Bot.ParseRequest(c.Request)
	if err != nil {
				...
				case *linebot.TextMessage:
					switch {
						case strings.HasPrefix(message.Text, "登録"):
							...
							if err != nil {
								utils.BotReplyMessage(event, "登録に失敗しました。")
							}
							utils.BotReplyMessage(event, todo.Title+"を登録しました。")

						case strings.HasPrefix(message.Text, "削除"):
							title := strings.TrimPrefix(message.Text, "削除　")
							err := todo_handler.Delete(event, title)
							if err != nil {
								utils.BotReplyMessage(event, "削除に失敗しました。")
							}
							utils.BotReplyMessage(event, message.Text+"を削除しました。")

						case strings.HasPrefix(message.Text, "一覧"):
							todos, err := todo_handler.Index(event)
							if err != nil {
								utils.BotReplyMessage(event, "一覧の取得に失敗しました。")
							}
							var titles []string
							for _, todo := range todos {
								titles = append(titles, todo.Title)
							}
							utils.BotReplyMessage(event, strings.Join(titles, "\n"))

						default:
							utils.BotReplyMessage(event, "登録、削除、一覧のいずれかを入力してください。")
					}
			}
		}
	}
